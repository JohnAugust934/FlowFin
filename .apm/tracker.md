...
| 5.2+ | Dispatched (fix: categorias offline) | frontend-agent | feature/pwa-offline |
...
- **5.4 PREPARADA** (Frontend, branch `feature/ui-export-lgpd-perfil`, worktree próprio criado): UI de export, exclusão de conta, Perfil dark. **Dispatch PAUSADO** — o usuário foi fazer deploy/teste antes de iniciar; o bus do frontend foi reaproveitado para o fix offline (5.2+). RETOMAR 5.4 após o fix offline fechar (reescrever o task.md do frontend com o prompt da 5.4 que está no histórico).
